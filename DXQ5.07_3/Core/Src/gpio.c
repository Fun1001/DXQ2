...
{
	static uint8_t oldkey1=0x00;
	static uint8_t keycnt1=0;
	uint8_t key=0x00;
	key+=(HAL_GPIO_ReadPin(K1_GPIO_Port,K1_Pin)==GPIO_PIN_RESET)?1:0;
	key+=(HAL_GPIO_ReadPin(K2_GPIO_Port,K2_Pin)==GPIO_PIN_RESET)?1:0;
  key+=(HAL_GPIO_ReadPin(K3_GPIO_Port,K3_Pin)==GPIO_PIN_RESET)?1:0;
	key+=(HAL_GPIO_ReadPin(K4_GPIO_Port,K4_Pin)==GPIO_PIN_RESET)?1:0;
	if((key>=4)&&key==oldkey1)
	{
		++keycnt1;
		if(2==keycnt1)
			return key;
