...
function device_data_analyze(dev)
		...
		--temp,ax,ay,az,gx,gy,gz,fAX,fAY,fAZ,warn
--#temp#ax#ay#az#gx#gy#gz#pitch#roll#yaw#warn#
--#24#000000#000000#000000#000000#000000#000000#10000#10000#10000#0#
--T:24.9, A: 15796, -1264,  3612, G: -2442, -1172, -2580, F: 69.0,-11.8,-11.0, W:2
--0：心跳
--temperature：两位整形温度数据
--humidity：两位整形湿度数据
--state：一位整形状态数据，0表示灯灭，1表示灯亮
--running：一位整形设备运转状态数据，1表示设备运转正常，0表示设备运转异常
--light：未知位数数据，所以放在最后
		add_val(t,"test",a,dev:bytes(1,s)) --添加到temp数据流
		add_val(t,"temp",a,dev:bytes(2,2)) --添加到temp数据流
		add_val(t,"ax",a,dev:bytes(5,6)) --添加到ax数据流
		add_val(t,"ay",a,dev:bytes(12,6)) --添加到ay数据流
		add_val(t,"az",a,dev:bytes(19,6)) --添加到az数据流
        add_val(t,"gx",a,dev:bytes(26,6)) --添加到gx数据流
		add_val(t,"gy",a,dev:bytes(33,6)) --添加到gy数据流
		add_val(t,"gz",a,dev:bytes(40,6)) --添加到gz数据流
		add_val(t,"pitch",a,dev:bytes(47,5)) --添加到pitch数据流
		add_val(t,"roll",a,dev:bytes(53,5)) --添加到gx数据流
		add_val(t,"yaw",a,dev:bytes(59,5)) --添加到gx数据流
		add_val(t,"warn",a,dev:bytes(65,1)) --添加到warn数据流
--		add_val(t,"wd_test",a,dev:bytes(1,2)) --t:数组；wd_test:数据流；a:时间戳;dev:bytes():返回字符串；wdtest：设备鉴权碼
--		add_val(t,"sd_test",a,dev:bytes(3,2)) --t:数组；sd_test:数据流；a:时间戳;dev:bytes():返回字符串；sdtest：设备鉴权碼
--		local light = dev:bytes(11,s-10)
--		if light <= 100 then
--			dev:send("led_on\r\n")
--		end
        dev:response()
