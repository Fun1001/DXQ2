...
{
       //接收数据处理
       //处理数据帧：  T:26.6, A:   470,   294, 14896, G:     2,    -3,    -3, F: -0.2, -0.1, -1.3, W:0
       ...
       if(str[0] == 'T' && str.length() >= 60)
       {
           ...
           switch(w)
           {
                default: ui->edit_warn->setText("无");
                     break;
                case 1: ui->edit_warn->setText("温度报警");
                     break;
                case 2: ui->edit_warn->setText("震动报警");
                     break;
                case 3: ui->edit_warn->setText("温度震动报警");
                     break;
           }
       }
       else if(str.startsWith("P1:") && str.length()>=20)   //参数帧
       {
           ...
           if(pos >= 0)
           {
                ...
                ui->spin_uptime->setValue(upstep /1000.0);
...
void MainWindow::ServerReadData()
{
    ...
    for(int i = 0; i < lstClient.length(); ++i)
    {
        ...
        //////////////////////////////////////////
        ///接收数据处理
        QString strRecv = QString(buffer);     //字节数组转换成字符串
...
void MainWindow::on_btn_setpara_clicked()       //设置参数
{
